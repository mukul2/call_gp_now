import 'dart:async';
import 'dart:convert';

import 'package:call_gp_now/view/patient/sharedActivitys.dart';
import 'package:flutter/cupertino.dart';
import 'package:shared_preferences/shared_preferences.dart';

String _baseUrl = "https://appointmentbd.com/api/";
String _baseUrl_image = "https://appointmentbd.com/";

class DoctorsListModelModel {
  static DoctorsListModelModel model = null;
  final StreamController<List> _Controller = StreamController<List>.broadcast();

  Stream<List> get outData => _Controller.stream;

  Sink<List> get inData => _Controller.sink;

  dataReload() {
    fetch().then((value) => inData.add(value));
  }

  void dispose() {
    _Controller.close();
  }

  static DoctorsListModelModel getInstance() {
    if (model == null) {
      model = new DoctorsListModelModel();
      return model;
    } else {
      return model;
    }
  }

  Future<List> fetch() async {
    Future<SharedPreferences> _prefs = SharedPreferences.getInstance();
    SharedPreferences prefs = await _prefs;
    String AUTH_KEY = prefs.getString("auth");
    String UID = prefs.getString("uid");
   // body = <String, String>{'user_type': "patient", 'id': UID};
    String apiResponse = await makePostReq("doctor-search", AUTH_KEY, body);

    return json.decode(apiResponse);
  }
}
